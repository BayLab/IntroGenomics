...
We first need to download the data. Open a terminal and navigate to your directory. Then use wget to download the data folder. We'll then use the tar command to un-compress it.
...
Change directory into the week9 folder. What do you see? 

The SNP data is in a vcf file, just like for previous weeks. You can take a peak at the top of the vcf file like this:
...
Open R Studio. The first thing we need to do is download a few different packages. 

Today we will be using the **SeqArray** and **SNPRelate** packages to read our data. You used these last week so they should already be installed. For the outlier analysis, we will use a package called **OutFLANK** which also needs the package **qvalue** to run (we call this a 'dependency').
...
Copy the lines in your script used to answer these
