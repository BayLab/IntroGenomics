...
> We will be using the iris dataset again which can be loaded with
> `data(iris)` and submit code and answers for the following questions:
> It is important to acknowledge that the field of genetics has been
> built on eugenics, and Fisher (author of this dataset) was a prominent
> geneticist and eugenicist. More information about this can be accessed
> here:
...
> -   Q2. How many of the irises are in this dataset belong to the
>     species versicolor?
...
> `sleep_total` is on the x (dependent variable) and `sleep_rem` si on
...
> > ## Exercise 1
> >
> > 1)  How many irises have sepals less than 5.5 cm? remember TRUE's
> >     are 1 and FALSE's are 0 and which() will only return the index
> >     of TRUE values, so we're counting how many there are

```{r, echo=T}
sum(iris[,'Sepal.Length']<5.5)
length(which(iris[,'Sepal.Length']<5.5))  
```

> > 2)  How many of the irises are in this dataset belong to the species
> >     versicolor?
